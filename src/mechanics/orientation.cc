...
Model OrientationController::model = Model();

void OrientationController::set_model(Model model1) {
  model = model1;
}

OrientationController::OrientationController(Car &c) : car(c) {
  target = eye<3>();
  eps_phi = 0.10f;
...
void OrientationController::step(float dt) {
  vec3 omega = dot(transpose(car.o), car.w);
  mat3 theta = dot(transpose(car.o), target);

  finished = angle_between(car.o, target) < eps_phi;
...
Car OrientationController::simulate() {

  Car car_copy = Car(car);
  auto copy = OrientationController(car_copy);
  copy.target = target;
