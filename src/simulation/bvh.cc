...
ray bvh< tri >::raycast_any(const ray & query_ray) const {
  ...
  *stack_ptr++ = -1;
  ...
  do {
    ...
    if (overlap_left && (left < num_leaves)) {
      ...
      if (::intersect(primitives[left], query_ray)) {
        ...
        return ray{ start, primitives[left].unit_normal() };
      }
    }
    ...
    if (overlap_right && (right < num_leaves)) {
      ...
      if (::intersect(primitives[right], query_ray)) {
        ...
        vec3 start = query_ray.start + alpha * query_ray.direction;
        return ray{ start, primitives[right].unit_normal() };
      }
    }
  ...
  return ray{ query_ray.start + query_ray.direction, vec3{0.0f, 0.0f, 1.0f} };

}

template <>
ray bvh< tri >::raycast_nearest(const ray & query_ray) const {
  ...
  *stack_ptr++ = -1;
    ...
    if (overlap_left && (left < num_leaves)) {
      ...
      if (::intersect(primitives[left], query_ray)) {
        float alpha = project_onto_tri(primitives[left], query_ray)[2];
				if (alpha < alpha_min) {
					alpha_min = alpha;
					normal = primitives[left].unit_normal();
				}
      }
    }
    ...
    if (overlap_right && (right < num_leaves)) {
      ...
      if (::intersect(primitives[right], query_ray)) {
        float alpha = project_onto_tri(primitives[right], query_ray)[2];
				if (alpha < alpha_min) {
					alpha_min = alpha;
					normal = primitives[right].unit_normal();
				}
      }
    }
  ...
  return ray{ query_ray.start + alpha_min * query_ray.direction, normal };
...
template std::vector < int > bvh< sphere >::intersect(const sphere &) const;
