...
// gets the parametric coordinates of the ray's intersection
// with the triangle (NOT THE WORLD COORDINATES)
vec3 project_onto_tri(const tri & a, const ray & r) {
...
bool intersect(const tri & a, const ray & r) {

	vec3 u_v_alpha = project_onto_tri(a, r);

	float u = u_v_alpha[0];
	float v = u_v_alpha[1];
	float w = 1.0f - u - v;
	float alpha = u_v_alpha[2];

	// the ray's intersection must be inside the triangle
	// and the distance along the ray must be in bounds
...
bool intersect(const aabb & box, const ray & r) {
	...
	// clip interval along the x-direction 
	if (n[0] != 0.0) {
		t1 = (box.min_x - p[0]) / n[0];
		t2 = (box.max_x - p[0]) / n[0];

		t_min = fmaxf(t_min, fminf(t1, t2));
		t_max = fminf(t_max, fmaxf(t1, t2));
	}

	// clip interval along the y-direction 
	if (n[1] != 0.0) {
		t1 = (box.min_y - p[1]) / n[1];
		t2 = (box.max_y - p[1]) / n[1];

		t_min = fmaxf(t_min, fminf(t1, t2));
		t_max = fminf(t_max, fmaxf(t1, t2));
	}

	// clip interval along the z-direction 
	if (n[2] != 0.0) {
		t1 = (box.min_z - p[2]) / n[2];
		t2 = (box.max_z - p[2]) / n[2];
